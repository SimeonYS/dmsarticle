...
class DmsarticlePipeline:

    # Database setup
    conn = sqlite3.connect('dmsarticle.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dmsarticle`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dmsarticle WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dmsarticle (date, title, link, content)"
